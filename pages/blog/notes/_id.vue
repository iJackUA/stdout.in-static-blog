...
export default {
  async asyncData ({ $content, params, $dateFns }) {
    const post = await $content('posts/en', params.slug).fetch()

    const [prev, next] = await $content('posts/en')
      .only(['title', 'slug'])
      .sortBy('date', 'asc')
      .surround(params.slug)
      .fetch()

    const dateFormatted = format(new Date(post.date), 'MMMM/dd/yyyy', { locale: enUS })
